...
import { ScrollView, Text, TouchableOpacity, View, StatusBar, Animated, Easing, ToastAndroid, ActivityIndicator } from 'react-native';
...
const DashboardScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    console.log('DashboardScreen: Mounted, user:', user ? user.id : 'null');
    if (!user) {
    ...
    database.ref(`users/${userId}`).once('value', snapshot => {
      ...
      if (!userData || !userData.role) {
        console.log('DashboardScreen: No user data or role found for user:', userId);
      ...
      database.ref('reports/approved').on('value', snapshot => {
        ...
        console.error('DashboardScreen: Error fetching approved reports:', error);
        ToastAndroid.show('Failed to load dashboard data. Please try again later.', ToastAndroid.BOTTOM);
        setError(error.message);
        setLoading(false);
      });
    }, error => {
      console.error('DashboardScreen: Error fetching user data:', error);
